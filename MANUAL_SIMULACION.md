...
3. Escribe comandos como si le hablaras a MIA. Prueba estos:
   ...
   | `soy el auditor, explicame como funcionas` | Entra en **modo auditor** y te explica su arquitectura |
   | `termina la auditoria` | Sale del modo auditor y vuelve a ser solo bartender |
   | `cambia de voz` | Cicla al siguiente proveedor de voz y lo anuncia |

   > **Modo auditor:** al decir "**soy el auditor**" MIA desbloquea información
   > técnica y te explica dónde está su cerebro (Groq), qué servicios usa, cómo se
   > conecta a la Raspberry Pi y cómo controla las bombas. Para salir di
   > "**termina la auditoría**" y vuelve a las restricciones de solo bar.
